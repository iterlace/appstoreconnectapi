...
class Resource(ABC):
    ...
    def __dir__(self):
        return ['id'] + list(self._data.get('attributes', {}).keys()) + list(
            self._data.get('relationships', {}).keys())
...
class BetaGroup(Resource):
    ...
    type = 'betaGroups'
    attributes = ['isInternalGroup', 'name', 'publicLink', 'publicLinkEnabled', 'publicLinkId',
                  'publicLinkLimit', 'publicLinkLimitEnabled', 'createdDate']
    relationships = {
        'app': {'multiple': False},
...
class AppStoreVersion(Resource):
    ...
    attributes = ['platform', 'appStoreState', 'copyright', 'earliestReleaseDate', 'releaseType',
                  'usesIdfa', 'versionString', 'createdDate', 'downloadable', 'createdDate']
    relationships = {
        'app': {'multiple': False},
...
class BetaAppLocalization(Resource):
    ...
    attributes = ['description', 'feedbackEmail', 'locale', 'marketingUrl', 'privacyPolicyUrl',
                  'tvOsPrivacyPolicy']
    relationships = {
        'app': {'multiple': False}
    }
...
class Build(Resource):
    endpoint = '/v1/builds'
    type = 'builds'
    attributes = ['expired', 'iconAssetToken', 'minOsVersion', 'processingState', 'version',
                  'usesNonExemptEncryption', 'uploadedDate', 'expirationDate']
    relationships = {
        'app': {'multiple': False},
...
class BuildBetaDetail(Resource):
    ...
    documentation = 'https://developer.apple.com/documentation/appstoreconnectapi/buildBetaDetail' \
					'/attributes'
...
class UserInvitation(Resource):
    ...
    documentation = 'https://developer.apple.com/documentation/appstoreconnectapi/userinvitation' \
					'/attributes'
...
class BundleId(Resource):
    ...
    documentation = 'https://developer.apple.com/documentation/appstoreconnectapi/bundleid' \
					'/attributes'


class Certificate(Resource):
    endpoint = '/v1/certificates'
    documentation = 'https://developer.apple.com/documentation/appstoreconnectapi/certificate' \
					'/attributes'
...
class Profile(Resource):
    ...
    documentation = 'https://developer.apple.com/documentation/appstoreconnectapi/profile' \
					'/attributes'
...
for name, obj in inspect.getmembers(sys.modules[__name__]):
    if inspect.isclass(obj) and issubclass(obj, Resource) and hasattr(obj,
                                                                      'type') and obj != Resource:
